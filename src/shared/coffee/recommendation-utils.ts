...
import { Coffee, DrinkStyle, FlavorNote, CoffeeScore } from "./models";

/**
 * Calculate how well a coffee's roast level matches user preference
 */
export const calculateRoastScore = (coffeeRoastLevel: number, userRoastLevel: number): number => {
  const roastDifference = Math.abs(coffeeRoastLevel - userRoastLevel);
  return Math.max(0, 30 - roastDifference * 6);
};

/**
 * Calculate how well a coffee's flavor notes match user preferences
 */
export const calculateFlavorScore = (
...
/**
 * Calculate how well a coffee matches drink style preference
 */
export const calculateDrinkStyleScore = (
  coffee: Coffee,
...
/**
 * Calculate bonus score based on coffee priority
 */
export const calculatePriorityBonus = (coffeePriority: number): number => {
  return 10 - coffeePriority;
};

/**
 * Find the best coffee matches based on user preferences
 */
